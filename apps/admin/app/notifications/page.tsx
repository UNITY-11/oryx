...
export default function NotificationsPage() {
  ...
  return (
    <div className="flex h-full bg-white rounded-[32px] border border-primary/10 shadow-sm overflow-hidden">
      
      {/* LEFT PANE - List */}
      <div className="w-full md:w-[400px] lg:w-[450px] shrink-0 border-r border-primary/10 flex flex-col bg-[#fdfaf8]">
        ...
        <div className="p-6 border-b border-primary/10">
          <div className="flex items-center justify-between mb-6">
            ...
            <button
              ...
              className="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider text-primary hover:text-primary-dark transition-colors disabled:opacity-40"
            >
              ...
              <button
                ...
                className={`px-4 py-1.5 rounded-full text-xs font-semibold uppercase tracking-wider transition-all duration-300 ${
                  filter === type
                    ...
                    : "bg-white text-text-secondary border border-primary/20 hover:border-primary/40 hover:text-primary"
                }`}
              >
                {type}
              </button>
        ...
        {/* List */}
        <div className="flex-1 overflow-auto scrollbar-hide divide-y divide-primary/5">
          {filtered.length === 0 ? (
              ...
              <div
                ...
                className={`flex gap-4 p-5 cursor-pointer transition-all duration-300 relative ${
                  selectedId === notification.id
                    ? "bg-white shadow-[inset_4px_0_0_0_rgba(200,169,156,1)]"
      ...
      <div className="flex-1 hidden md:flex flex-col bg-white overflow-hidden relative">
        {selectedNotif ? (
          <div className="flex-1 overflow-auto scrollbar-hide flex flex-col">
            ...
            {/* Detail Header */}
            <div className="p-10 border-b border-primary/5 bg-gradient-to-b from-primary/5 to-transparent flex items-start gap-5">
              <div className={`w-16 h-16 rounded-[1.25rem] flex items-center justify-center text-white shrink-0 shadow-md shadow-primary/10 ${
                selectedNotif.type === 'Booking' ? 'bg-[#c8a99c]' :
                selectedNotif.type === 'Stock' ? 'bg-[#d6bba7]' :
                'bg-[#e3d1c4]'
              }`}>
                {getIcon(selectedNotif.type)}
              </div>
              <div className="flex-1">
                <span className="inline-block px-3 py-1 rounded-full bg-white border border-primary/10 text-[10px] font-bold uppercase tracking-wider text-primary mb-3 shadow-sm">
                  {selectedNotif.type} Notification
                </span>
                <h2 className="text-3xl font-serif text-primary-dark mb-2">{selectedNotif.title}</h2>
                <p className="text-text-secondary/60 text-sm flex items-center gap-2">
            ...
            <div className="p-10 flex-1">
              ...
              {selectedNotif.type === "Booking" && selectedNotif.bookingData && (
                <div className="grid grid-cols-1 xl:grid-cols-2 gap-8 max-w-4xl">
                  
                  {/* Customer Card */}
                  <div className="bg-[#fdfaf8] rounded-3xl p-6 border border-primary/10">
                    <h4 className="text-xs font-bold uppercase tracking-wider text-text-secondary mb-4">Customer Details</h4>
                    <div className="flex items-center gap-4 mb-6">
                      <div className="w-12 h-12 rounded-full bg-primary/20 text-primary-dark flex items-center justify-center text-lg font-serif">
                    ...
                    <div className="flex items-center gap-2">
                      <button className="flex-1 p-3 bg-primary/5 hover:bg-primary/10 text-primary rounded-2xl transition-colors flex justify-center items-center group" title="Call">
                        <Phone className="w-4 h-4 group-hover:scale-110 transition-transform" />
                      </button>
                      <button className="flex-1 p-3 bg-primary/5 hover:bg-primary/10 text-primary rounded-2xl transition-colors flex justify-center items-center group" title="Message">
                        <MessageSquare className="w-4 h-4 group-hover:scale-110 transition-transform" />
                      </button>
                      <button className="flex-1 p-3 bg-primary/5 hover:bg-primary/10 text-primary rounded-2xl transition-colors flex justify-center items-center group" title="WhatsApp">
                        <MessageCircle className="w-4 h-4 group-hover:scale-110 transition-transform" />
                      </button>
                      <button className="flex-1 p-3 bg-primary/5 hover:bg-primary/10 text-primary rounded-2xl transition-colors flex justify-center items-center group" title="Email">
                  ...
                  {/* Session Card */}
                  <div className="bg-[#fdfaf8] rounded-3xl p-6 border border-primary/10 flex flex-col">
                    <div className="flex justify-between items-center mb-4">
                      <h4 className="text-xs font-bold uppercase tracking-wider text-text-secondary">Session Details</h4>
                    ...
                    <div className="space-y-4 flex-1">
                      <div>
                        <p className="text-sm font-medium text-primary-dark">{selectedNotif.bookingData.serviceName}</p>
                      </div>
                    ...
                    {selectedNotif.bookingData.status === "Pending" && (
                      <div className="flex gap-3 mt-6 pt-6 border-t border-primary/10">
                        <button 
                          ...
                          className="flex-1 bg-primary text-white py-3 rounded-2xl text-sm font-semibold shadow-md shadow-primary/20 hover:shadow-lg hover:shadow-primary/30 transition-all hover:-translate-y-0.5"
                        >
                          Confirm Booking
                        </button>
                        <button className="px-6 py-3 bg-white text-text-secondary border border-primary/20 rounded-2xl text-sm font-semibold hover:bg-gray-50 transition-colors">
                          Decline
                        </button>
                      </div>
                    )}
              ...
              {selectedNotif.type !== "Booking" && selectedNotif.actionUrl && (
                <Link
                  ...
                  className="inline-flex items-center justify-center gap-2 bg-primary text-white px-8 py-3.5 rounded-full text-sm font-semibold shadow-md shadow-primary/20 hover:shadow-lg hover:shadow-primary/30 transition-all hover:-translate-y-0.5"
                >

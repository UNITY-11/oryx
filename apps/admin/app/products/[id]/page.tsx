...
export default function ProductDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const original = MOCK_PRODUCTS.find((p) => p.id === id) ?? null;
  const [product, setProduct] = useState<Product | null>(original ? { ...original } : null);
  ...
  if (!product) {
    return (
      <div className="flex flex-col items-center justify-center h-full text-text-secondary">
        ...
        <button onClick={() => router.push("/products")} className="text-primary underline text-sm">
          Back to Products
        </button>
      </div>
    );

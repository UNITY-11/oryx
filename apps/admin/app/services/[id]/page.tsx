...
export default function ServiceDetailPage({ params }: { params: { id: string } }) {
  ...
  const original = MOCK_SERVICES.find((s) => s.id === params.id) ?? null;

  const [service, setService] = useState<Service | null>(original ? { ...original } : null);
  ...
  if (!service) {
    return (
      <div className="flex flex-col items-center justify-center h-full text-text-secondary">
        ...
        <button onClick={() => router.push("/services")} className="text-primary underline text-sm">
          Back to Services
        </button>
      </div>
    );
            ...
            <div className="flex-1 space-y-8 min-w-0">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-3 rounded-2xl border border-primary/20 bg-transparent focus:outline-none focus:border-primary text-primary-dark font-medium text-base"
                  />
                </div>
                <div>
              ...
              <div>
                ...
                <div className="rounded-2xl border border-primary/10 overflow-hidden">
                  {/* Header */}
                  <div className="grid grid-cols-[1fr_120px_120px_40px] gap-0 bg-[#fcf4f0] text-[10px] uppercase tracking-wider text-text-secondary px-4 py-2.5 border-b border-primary/10">
                    <span>Label</span>
                    <span>Price (QAR)</span>
                  ...
                  {service.pricingTiers.map((tier, i) => (
                    <div
                      key={tier.id}
                      className={`grid grid-cols-[1fr_120px_120px_40px] gap-0 items-center px-4 py-2 ${i > 0 ? "border-t border-primary/5" : ""}`}
                    >
                      <input
                        value={tier.label}
                        onChange={(e) => updateTier(tier.id, "label", e.target.value)}
                        placeholder="e.g. 60 min"
                        className="bg-transparent focus:outline-none text-primary-dark text-sm py-1.5"
                      />
                      <input
                        type="number"
                        value={tier.price || ""}
                        onChange={(e) => updateTier(tier.id, "price", Number(e.target.value))}
                        className="bg-transparent focus:outline-none text-primary-dark text-sm py-1.5 font-medium"
                      />
                      <input
                        type="number"
                        value={tier.duration || ""}
                        onChange={(e) => updateTier(tier.id, "duration", Number(e.target.value))}
                        className="bg-transparent focus:outline-none text-primary-dark text-sm py-1.5"
                      />
                      <button
                ...
                {service.addons.length === 0 ? (
                  ...
                  <div className="rounded-2xl border border-primary/10 overflow-hidden">
                    ...
                    {service.addons.map((addon, i) => (
                      <div
                        key={addon.id}
                        className={`grid grid-cols-[1fr_120px_120px_40px] items-center px-4 py-2 ${i > 0 ? "border-t border-primary/5" : ""}`}
                      >
                        <input
                          value={addon.name}
                        ...
                        <input
                          ...
                          onChange={(e) => updateAddon(addon.id, "duration", Number(e.target.value))}
                          className="bg-transparent focus:outline-none text-primary-dark text-sm py-1.5"
                        />
